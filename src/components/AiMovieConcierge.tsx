...
function formatCountryBadge(name?: string): string {
  ...
  if (n.includes("mỹ") || n.includes("âu") || n.includes("us")) return "🇺🇸 " + name;
...
function AiMovieSection({
  ...
  const visibleMovies = isExpanded ? filteredMovies : filteredMovies.slice(0, 6);
    ...
    <div className="mt-3 pt-3 border-t border-white/10 space-y-2.5">
      ...
      <div className="bg-black/50 rounded-xl p-2.5 border border-white/10 space-y-2">
        <div className="flex items-center justify-between gap-2 flex-wrap">
          ...
          {(selectedCountry !== "all" || selectedActor !== "all" || actorSearch) && (
            <button
              type="button"
              onClick={() => {
        ...
        {(actorList.length > 0 || movies.some((m) => m.actors && m.actors.length > 0)) && (
              ...
              <div className="flex items-center gap-1.5 overflow-x-auto scrollbar-none pb-0.5">
                {actorList.map(([actName, count]) => (
                  <button
                    ...
                    type="button"
                    onClick={() => {
                      setSelectedActor(selectedActor === actName ? "all" : actName);
                      setActorSearch("");
                    }}
      ...
      {visibleMovies.length === 0 ? (
        <div className="text-center py-4 text-gray-400 text-xs bg-black/20 rounded-xl border border-white/5">
          Không tìm thấy phim nào khớp với bộ lọc đã chọn. Hãy thử chọn quốc gia hoặc diễn viên khác nhé!
        </div>
      ) : (
        ...
        >
          {isExpanded ? (
            ...
            <>
              ...
              <span>Xem thêm {filteredMovies.length - 6} phim gợi ý khác ⬇</span>
            </>
          )}
        </button>
      )}
              ...
              <div className="flex items-center gap-2.5 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-[11px] text-gray-400 truncate flex items-center gap-1.5">
                    ...
                    <span>Nana gợi ý phim thông minh theo cảm xúc & gu xem của bạn</span>
                  </p>
                </div>
              </div>
