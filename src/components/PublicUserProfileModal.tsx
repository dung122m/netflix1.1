    ...
    >
      ...
      >
        ...
        <div className="relative pt-1 sm:pt-0">
          <div className="flex flex-col sm:flex-row items-center sm:items-start text-center sm:text-left gap-3 sm:gap-5 px-1 sm:px-0 pr-12 sm:pr-0">
            {/* AVATAR TRÒN CÓ VIỀN CẤP ĐỘ VIP */}
            <div className="relative flex-shrink-0 mx-auto sm:mx-0">
              <div className="w-16 h-16 sm:w-22 sm:h-22 rounded-full overflow-hidden bg-gradient-to-br from-indigo-600 via-purple-600 to-rose-600 p-0.5 shadow-xl ring-2 ring-white/20">
                <UserAvatar
                  src={avatarUrl || undefined}
                  name={profile.displayName}
                  seed={profile.uid || profile.displayName}
                  sizeClassName="w-full h-full text-xl sm:text-3xl font-black"
                />
              </div>
              <div
                className={`absolute -bottom-1 -right-1 px-1.5 sm:px-2 py-0.5 rounded-full text-[9px] sm:text-[10px] font-black border flex items-center gap-1 shadow-lg bg-zinc-950 ${levelInfo.colorClass}`}
              >
            ...
            <div className="flex-1 min-w-0 space-y-1.5 sm:space-y-2 w-full">
              <div className="flex flex-wrap items-center justify-center sm:justify-start gap-1.5 sm:gap-2">
                <h3 className="text-base sm:text-2xl font-black text-white tracking-wide break-words text-center sm:text-left">
                  {profile.displayName}
                </h3>
                {isAdmin ? (
                  <span className="inline-flex items-center gap-1 px-2 sm:px-2.5 py-0.5 rounded-full text-[10px] sm:text-[11px] font-black bg-rose-500/20 text-rose-300 border border-rose-500/40 whitespace-nowrap flex-shrink-0">
                    <ShieldCheck className="w-3.5 h-3.5" />
                    Quản Trị Viên
                  </span>
                ) : (
                  <span className="inline-flex items-center gap-1 px-1.5 sm:px-2 py-0.5 rounded-full text-[10px] sm:text-[10.5px] font-bold bg-amber-500/15 text-amber-300 border border-amber-500/30 whitespace-nowrap flex-shrink-0">
              ...
              {profile.bio ? (
                ...
                <p className="text-[11px] sm:text-xs text-zinc-500 text-center sm:text-left">Thành viên chưa thêm lời giới thiệu.</p>
              )}

              {/* HUY HIỆU SỞ HỮU */}
              <div className="flex flex-wrap items-center justify-center sm:justify-start gap-1 sm:gap-1.5 pt-0.5">
                {profile.badges && profile.badges.length > 0 ? (
                  profile.badges.map((badge, idx) => (
                    <span
                      key={idx}
                      className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] sm:text-[11px] font-bold bg-white/5 border border-white/10 text-amber-300 shadow-sm whitespace-nowrap flex-shrink-0"
                    >
                  ...
                  <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] sm:text-[10.5px] font-medium bg-white/5 border border-white/10 text-zinc-400 whitespace-nowrap">
                    🍿 Mọt Phim Đêm
                  </span>
                )}
              </div>
        ...
        <div className="grid grid-cols-3 gap-1.5 sm:gap-3">
          <div className="p-1.5 sm:p-3 rounded-xl sm:rounded-2xl bg-zinc-900/60 border border-white/10 text-center space-y-0.5 flex flex-col justify-center items-center min-w-0 overflow-hidden">
            <div className="flex items-center justify-center gap-1 text-amber-400">
              <Clock className="w-3.5 h-3.5 flex-shrink-0" />
              <span className="text-[9.5px] sm:text-xs font-semibold uppercase tracking-wider text-zinc-400 whitespace-nowrap">
                Đã Cày
              </span>
            </div>
            <div className="text-sm sm:text-xl font-black text-white truncate max-w-full">{watchHours}h</div>
            <div className="text-[9px] sm:text-[10px] text-zinc-500 truncate max-w-full">{watchMins.toLocaleString()} phút</div>
          </div>
          ...
          <div className="p-1.5 sm:p-3 rounded-xl sm:rounded-2xl bg-zinc-900/60 border border-white/10 text-center space-y-0.5 flex flex-col justify-center items-center min-w-0 overflow-hidden">
            ...
            <div className="text-xs sm:text-base font-black text-white truncate max-w-full px-0.5">
              {levelInfo.levelName}
            </div>
            <div className="text-[9px] sm:text-[10px] text-zinc-500 truncate max-w-full">{levelInfo.badgeIcon} Cấp</div>
          </div>
          ...
          <div className="p-1.5 sm:p-3 rounded-xl sm:rounded-2xl bg-zinc-900/60 border border-white/10 text-center space-y-0.5 flex flex-col justify-center items-center min-w-0 overflow-hidden">
            ...
            <div className="text-sm sm:text-xl font-black text-white truncate max-w-full">{collections.length}</div>
            <div className="text-[9px] sm:text-[10px] text-zinc-500 whitespace-nowrap">công khai</div>
          </div>
        </div>

        {/* TABS SELECTOR (TỔNG QUAN & BỘ SƯU TẬP) */}
        <div className="flex items-center gap-1 p-1 bg-zinc-900/90 rounded-2xl border border-white/10 text-[11px] sm:text-xs font-bold">
          <button
            type="button"
            onClick={() => setActiveTab("overview")}
            className={`flex-1 py-2 px-2 sm:px-3 rounded-xl transition cursor-pointer flex items-center justify-center gap-1.5 whitespace-nowrap ${activeTab === "overview"
          ...
          <button
            ...
            className={`flex-1 py-2 px-2 sm:px-3 rounded-xl transition cursor-pointer flex items-center justify-center gap-1.5 whitespace-nowrap ${activeTab === "collections"
              ? "bg-netflix-red text-white shadow-lg shadow-rose-950/40"
              : "text-zinc-400 hover:text-white hover:bg-white/5"
              }`}
          >
        ...
        <div className="space-y-3.5 min-h-[140px]">
          ...
          {activeTab === "overview" && (
            <div className="space-y-3.5">
              ...
              <div className="p-3 sm:p-4 rounded-2xl bg-zinc-900/40 border border-white/5 space-y-2">
                ...
                {profile.favoriteGenres && profile.favoriteGenres.length > 0 ? (
                  <div className="flex flex-wrap gap-1 sm:gap-2">
                    {profile.favoriteGenres.map((genre, gIdx) => (
                      <span
                        key={gIdx}
                        className="px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-xl text-[10.5px] sm:text-xs font-semibold bg-rose-500/10 text-rose-300 border border-rose-500/20 whitespace-nowrap"
                      >
                        🎬 {genre}
                      </span>
                    ))}
              ...
              {levelInfo.nextLevelName && (
                <div className="p-3 sm:p-4 rounded-2xl bg-gradient-to-r from-zinc-900 to-zinc-950 border border-white/10 space-y-2">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between text-xs gap-1">
                    ...
                    <span className="text-zinc-400 font-mono text-[10.5px] sm:text-xs">
              ...
              {collections.length > 0 ? (
                collections.map((col) => (
                  <Link
                    ...
                    onClick={() => setIsOpen(false)}
                    className="flex items-center justify-between p-2.5 sm:p-3.5 rounded-xl sm:rounded-2xl bg-zinc-900/50 border border-white/5 hover:border-white/15 hover:bg-zinc-900/80 transition group cursor-pointer"
                  >
                    <div className="space-y-0.5 min-w-0 flex-1 pr-2">
                      <div className="text-xs sm:text-sm font-bold text-white group-hover:text-rose-400 transition truncate">
                        {col.name}
                      </div>
                      <div className="text-[10.5px] sm:text-[11px] text-zinc-400 truncate">
                        {col.description || "Bộ sưu tập phim yêu thích"}
                      </div>
                    </div>
                    <div className="flex items-center gap-1.5 flex-shrink-0 text-[11px] sm:text-xs font-semibold text-zinc-400">
