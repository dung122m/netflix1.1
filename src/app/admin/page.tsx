      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        {activeTab === "reports" && (
          <div className="space-y-4">
            ...
            <div className="p-4 rounded-2xl bg-zinc-900/60 border border-white/10 backdrop-blur-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
              {/* Search input */}
              <div className="relative flex-1">
                <Search size={16} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-400" />
                <input
                  type="text"
                  placeholder="Tìm kiếm theo tên phim, slug, email người báo, mô tả lỗi..."
                  value={reportSearchQuery}
                  onChange={(e) => setReportSearchQuery(e.target.value)}
                  className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-black/50 border border-white/10 text-white placeholder-gray-500 text-xs focus:outline-none focus:border-netflix-red/60 transition"
                />
              </div>
              ...
              <div className="flex items-center gap-1.5 flex-wrap">
                ...
                >
                  ...
                  <span>Chờ xử lý ({errorReports.filter((r) => r.status === "pending").length})</span>
                </button>
            ...
            {filteredErrorReports.length === 0 ? (
              <div className="p-12 rounded-2xl bg-zinc-900/40 border border-white/5 text-center text-gray-500 space-y-2">
                ...
                <p className="text-xs text-gray-500">
                  {reportSearchQuery ? "Không tìm thấy kết quả phù hợp với bộ lọc tìm kiếm." : "Toàn bộ phim đang hoạt động ổn định hoặc chưa có phản hồi lỗi mới."}
                </p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5">
                {filteredErrorReports.map((report) => {
                  const issueBadgeMap: Record<string, { label: string; color: string }> = {
                    broken_link: { label: "Hỏng video / Không phát được", color: "bg-red-500/20 text-red-300 border-red-500/30" },
                    audio_issue: { label: "Lỗi âm thanh / Mất tiếng", color: "bg-amber-500/20 text-amber-300 border-amber-500/30" },
                    subtitle_issue: { label: "Lỗi phụ đề / Lệch sub", color: "bg-blue-500/20 text-blue-300 border-blue-500/30" },
                    wrong_episode: { label: "Sai tập phim / Trùng lặp", color: "bg-purple-500/20 text-purple-300 border-purple-500/30" },
                    other: { label: "Sự cố khác", color: "bg-zinc-700/50 text-gray-300 border-zinc-600/40" },
                  };
                  const badge = issueBadgeMap[report.issueType] || { label: report.issueType, color: "bg-zinc-700/50 text-gray-300 border-zinc-600/40" };
                  ...
                  return (
                    ...
                    >
                      <div className="space-y-2.5">
                        {/* Header: Title, Episode, and Status Badge */}
